...
def run() -> List[Dict]:
                ...
                for n_train_iters in N_TRAIN_ITERS_SWEEP:
                    ...
                    for label in active_ccld:
                        for sampler in SAMPLER_SWEEP:
                            key = f"{label}@{sampler}"
                            print(f"\nN_orig={N_orig} gt_seed={gt_seed} (N_eff={N_eff}) steps={n_diff_steps} iters={n_train_iters} condition={key}")
                            per_seed = {}
                            for seed in SEEDS:
                                metrics, generated = pns.train_one_seed(
                                    N_eff, coupling, seed, label=f"sweep/{key}/gt{gt_seed}/steps{n_diff_steps}/iters{n_train_iters}",
                                ...
                                per_seed_rows.append({"N_orig": N_orig, "gt_seed": gt_seed, "n_diff_steps": n_diff_steps, "n_train_iters": n_train_iters, "sampler": sampler, "condition": label, "seed": seed, **metrics})
                            ...
                            summary_rows.append({
                                "N_orig": N_orig, "gt_seed": gt_seed, "n_diff_steps": n_diff_steps, "n_train_iters": n_train_iters, "sampler": sampler, "condition": label,
                    ...
                    for label in active_baselines:
                        ...
                        for seed in SEEDS:
                            ...
                            per_seed[seed] = metrics
                            per_seed_rows.append({"N_orig": N_orig, "gt_seed": gt_seed, "n_diff_steps": n_diff_steps, "n_train_iters": n_train_iters, "sampler": "euler", "condition": label, "seed": seed, **metrics})
                            print(f"  seed={seed}: kl={metrics['kl_divergence']:.4f} cross_asym_mae={metrics['cross_asym_mae']:.4f}")
                        per_seed_by_key[key] = per_seed

                        cond_summary = aggregate_over_seeds(per_seed)
                        summary_rows.append({
                            "N_orig": N_orig, "gt_seed": gt_seed, "n_diff_steps": n_diff_steps, "n_train_iters": n_train_iters, "sampler": "euler", "condition": label,
                            "kl_mean": cond_summary["kl_divergence"]["mean"], "kl_std": cond_summary["kl_divergence"]["std"],
                            "cross_asym_mae_mean": cond_summary["cross_asym_mae"]["mean"], "cross_asym_mae_std": cond_summary["cross_asym_mae"]["std"],
                        })
                    ...
                    if "symmetric_only" in active_ccld:
                        for sampler in SAMPLER_SWEEP:
                            ...
                            for label in active_ccld:
                                ...
                                for metric, s in sig.items():
                                    sig_rows.append({"N_orig": N_orig, "gt_seed": gt_seed, "n_diff_steps": n_diff_steps, "n_train_iters": n_train_iters, "comparison": f"{key}_vs_{baseline_key}", "metric": metric, **s})
                        ...
                        if euler_baseline_key in per_seed_by_key:
                            ...
                            for label in active_baselines:
                                ...
                                for metric, s in sig.items():
                                    sig_rows.append({"N_orig": N_orig, "gt_seed": gt_seed, "n_diff_steps": n_diff_steps, "n_train_iters": n_train_iters, "comparison": f"{key}_vs_{euler_baseline_key}", "metric": metric, **s})
    ...
    print("\n\nSUMMARY")
    print(f"{'N':>3} {'gt':>3} {'steps':>5} {'iters':>6} {'sampler':>7} {'condition':>22} {'KL':>10} {'asym_mae':>10}")
    for row in summary_rows:
        print(f"{row['N_orig']:>3} {row['gt_seed']:>3} {row['n_diff_steps']:>5} {row['n_train_iters']:>6} {row['sampler']:>7} {row['condition']:>22} {row['kl_mean']:>10.4f} {row['cross_asym_mae_mean']:>10.4f}")
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--gt-seeds", default="0")

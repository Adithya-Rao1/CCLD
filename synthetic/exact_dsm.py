...
def _extract_Avx_Avv_coupled_gamma(
    ...
    K_self = [[torch.tensor([[k_reference]])] for _ in range(N)]
    K_global = [torch.tensor([[k_reference]]) for _ in range(N)]
...
def sample_and_tikhonov_score_target(Z0: torch.Tensor, Phi_t: torch.Tensor, Sigma_t: torch.Tensor, N: int, lam: float, jitter: float = 1e-8):
    B = Z0.shape[0]
    mean = Z0 @ Phi_t.T
    Sigma_reg = Sigma_t + jitter * torch.eye(2 * N)
    L = torch.linalg.cholesky(Sigma_reg)
    eps = torch.randn(B, 2 * N)
    Zt = mean + eps @ L.T
    ...
    Svx = Sigma_t[N:, :N]
    Svv = Sigma_t[N:, N:]
    Sxx_inv = torch.linalg.inv(Sxx + jitter * torch.eye(N))
    ...
    reg_precision = torch.linalg.inv(Var_V_given_X + lam * torch.eye(N))
